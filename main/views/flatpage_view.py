...
class FlatPageView(View):
    ...
    def _not_authorized(self) -> bool:
        return self.flatpage.registration_required and not self.request.user.is_authenticated

    def _get_template(self) -> Template:
        if self.flatpage.template_name:
            return loader.select_template((self.flatpage.template_name, DEFAULT_TEMPLATE))
        else:
