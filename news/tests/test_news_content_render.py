...
class TestNewsContentRender(TestCase):
    ...
    def test_render_news_instance(self, kwargs):
        ...
        news = News.objects.prefetch_related("newscover", "newscontentimage_set").get(
            **kwargs
        )
        response = self.client.get(url, follow=True)

        self.assertContains(
            response, news.title, msg_prefix=f"Page should contain at {url}"
        )
        self.assertContains(
            response, news.text, msg_prefix=f"Page should contain at {url}"
        )
        self.assertContains(
            response,
            do_title(news.date.strftime("%Y")),
            msg_prefix=f"Page should contain at {url}",
        )
        self.assertContains(
            response,
            do_title(news.date.strftime("%m")),
            msg_prefix=f"Page should contain at {url}",
        )
        self.assertContains(
            response,
            do_title(news.date.strftime("%-d")),
            msg_prefix=f"Page should contain at {url}",
        )
        self.assertNotContains(
            response, news.hidden, msg_prefix=f"Page should not contain at {url}"
        )
